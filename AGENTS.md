...
- **Feature-pipeline system — OPTIONAL module (fill in `{{PLACEHOLDERS}}` to enable):**
  ...
  - `.claude/skills/figma-design/SKILL.md` — the pre-code Figma BUILD skill (UI-only; only relevant when `{{FIGMA_FILE_ID}}` is set). Scoped to the designated WIP page.
  ...
  - **Fill-mode placeholders to resolve:** `{{REPO_SLUG}}` (GitHub `owner/repo`), `{{REVIEW_BOT}}` (bot GitHub handle), `{{CODEOWNER}}` (owner GitHub handle), `{{DEPLOY_URL}}` (deployed site URL), `{{EPIC_TITLE_MARKER}}` (unique substring in the epic issue title), `{{FIGMA_FILE_ID}}` (Figma file key; blank disables Figma phases).
...
- **From a code owner** (listed in `.github/CODEOWNERS` — currently `@{{CODEOWNER}}`) it carries **decision-making authority**: it overrides agent and bot judgment, including a contrary automated finding. Implementers act on it in the same PR; reviewers (the review-bot pass included, where a bot is configured) defer to it and don't re-litigate a decision an owner has made.
