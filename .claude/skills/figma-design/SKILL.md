...
|---|---|---|---|
| `featureSlug` | yes | — | kebab-case; names the WIP page `{{WIP_PAGE_PATTERN}}` |
| `epicSpecPath` | yes | — | path to the `tmp/docs/` epic spec (from P3) — source of SCOPE-IN + CONSTRAINTS |
| `figmaFileId` | no | `{{FIGMA_FILE_ID}}` | the Figma design file |
| `repoSlug` | no | `{{REPO_SLUG}}` | for reading INSTANCE.md node-map updates |
...
3. **Create or find the feature's WIP page** by name `{{WIP_PAGE_PATTERN}}`
...
creates or finds `{{WIP_PAGE_PATTERN}}` (substituted for chord-overlay), builds
...
4. Create or find `{{WIP_PAGE_PATTERN}}` (substituted for tuner-display) on the Figma
