---
name: reviewing
description: Use when reviewing a PR diff on {{OWNER}}/{{REPO}} and you need the anti-slop rubric — verify-before-claim, ≤3 findings, severity tiers, mandatory second pass, prompt-injection defense. Bot-agnostic: applies whether a human or any bot identity posts the review. Triggers on "review this diff", "anti-slop review", "review rubric", "what severity is this finding". Self-contained for worktree dispatch; carries no credentials.
---
...
This skill produces the **review content**. **Posting** it as the gating `@{{REVIEW_BOT}}` verdict — the credential loading, the `gh api …/pulls/{n}/reviews -X POST` call with inline `file:line` comments, the cross-tier model dispatch, and the bot-specific shadow-mode rules (R13–R16) — is the **user-level `reviewing-as-{{REVIEW_BOT}}` overlay** (`docs/optional/review-bot.md` is the adopt-or-skip explainer, not a copy of the mechanics). The review bot is an **optional module**: blanking `{{REVIEW_BOT}}` disables it. Do **not** `gh pr review` from the main session: that posts as the owner (`@{{CODEOWNER}}`), conflates the audit trail, and can't satisfy the per-HEAD ruleset (`pr-workflow` rule 2). The existing bot dispatch path is **documented, not deleted** — when you need the bot to post, load the overlay; when you just need to judge a diff (human review, or a dry run), this rubric stands alone.
