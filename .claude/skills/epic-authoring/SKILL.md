...
- **GitHub repo slug for this instance is `{{REPO_SLUG}}`.** Replace `{{REPO_SLUG}}` with your repo's actual `owner/repo` value when filling this template.
...
| `featureSlug` | string (kebab-case) | Yes | Output dir name and issue label prefix. Example: `user-auth`, `reporting-dashboard`, `notification-engine`. |
...
| `repoSlug` | string | Optional | GitHub repo slug. Default: `{{REPO_SLUG}}`. |
...
- **Fill `{{REPO_SLUG}}` with your repo's actual `owner/repo` slug** before using this template in a new instance.
...
<example>
Context: User has run the front-door brainstorm for a new feature and has a brief at tmp/docs/<feature>-brief.md.
user: "Author the epic spec for the <feature>. Brief is at tmp/docs/<feature>-brief.md."
assistant: "I'm using the epic-authoring skill to research and synthesize a feature epic spec into tmp/docs/. I'll read the brief, invoke deep-research with the PROBLEM framing, and synthesize the epic spec to tmp/docs/<feature>/epic-spec.md."
</example>

<example>
Context: The feature pipeline engine is running phase(2) for a new feature.
user: "Run epic-authoring for featureSlug=<feature>, brainstormBriefPath=tmp/docs/<feature>-brief.md, contextPaths=[DESIGN.md]"
assistant: "Announcing: I'm using the epic-authoring skill. Reading DESIGN.md for domain context, then reading the brief, invoking deep-research on the problem space, and writing the epic spec to tmp/docs/<feature>/epic-spec.md."
</example>

<example>
Context: User wants a research-backed epic before filing child issues.
user: "I need an epic spec before filing child issues for the <feature-name> feature. Can you research this and produce the spec?"
assistant: "This is exactly what the epic-authoring skill covers — I'll research the domain via deep-research, synthesize the output against the brief's SCOPE/CRITERIA, and write the structured epic spec to tmp/docs/<featureSlug>/epic-spec.md, then surface it for issue-authoring fan-out."
</example>
...
user: "Write the implementation issue for the <specific-component> improvement."
...
user: "Review epic issue #N before we start implementation."
...
1. "Author the epic spec for the <feature> feature"
2. "Research and write the epic for <feature>"
3. "Run epic-authoring for featureSlug=<feature>"
...
7. "Produce the epic spec for issue fan-out on the <feature> feature"
8. "Build the epic for the <feature> feature from the brief"
9. "Run the research phase for the epic and write to tmp/docs/"
10. "Write the feature epic for <feature>"

### Should NOT trigger (near-misses, ≥8)

1. "Write the implementation issue for <component>" → issue-authoring
2. "Review epic issue #N before implementation" → issue-plan-review
3. "Bootstrap and validate the repo instance" → project-bootstrap
4. "Create a PR for the feature branch" → creating-prs / pr-workflow
5. "Review the epic-authoring skill PR" → reviewing-pipeline-assets
6. "What is the Figma file ID for this repo?" → project-bootstrap / INSTANCE.md lookup
7. "Run the full feature pipeline for <feature>" → feature-pipeline workflow engine
8. "Post a plan-review comment on issue #N" → issue-plan-review
9. "Merge the epic PR once the review passes" → merge workflow skill
...
This file is part of the `{{REPO_SLUG}}` instance of the agentic-seed template. Replace the following when filling this template for a new product:

| Placeholder | Replace with |
|---|---|
| `{{REPO_SLUG}}` | Your GitHub `owner/repo` slug (e.g. `acme/my-product`) |
| `{{BOT_IDENTITY}}` | Your review bot's GitHub identity (e.g. `@mybot`) |
| `{{LABEL_SCHEME}}` | Your repo's issue label set (e.g. `pipeline`, `skill`, `review-tooling`) |
| `{{FIGMA_FILE_ID}}` | Your Figma design file ID (or remove the Figma-phase references if unused) |
| `{{WIP_PAGE_PATTERN}}` | Your Figma WIP page naming convention (or remove if unused) |
| `{{VERDICT_TOKEN}}` | Your pipeline gate's approval token string |
