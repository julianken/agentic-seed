...
- **Repo slug:** `{{REPO_SLUG}}`; local folder is `{{LOCAL_FOLDER}}/`.
...
- **No human pauses mid-run.** The engine runs fully autonomously. All review gates are dispatched as fresh-context `@{{REVIEW_BOT}}` subagents. Blockers go to a RESOLUTION sub-workflow (diagnose→fix→verify). The engine escalates a `HIL:` note only for a genuine info-need or a truly-unresolvable problem (≥2 failed RESOLUTION cycles on the same blocker, or a missing arg only the owner can supply).
...
- **Bot identity:** `@{{REVIEW_BOT}}` posts reviews; the main session never runs `gh pr review` (that posts as `@julianken` and fails the per-HEAD branch ruleset).
...
| `repoSlug` | string | **required** | — | GitHub `owner/repo` slug. For this instance: `{{REPO_SLUG}}`. |
...
| 4 | Figma design review (UI-only) | `@{{REVIEW_BOT}}` loaded with `reviewing-figma-designs` reviews the WIP frames; writes `Figma-Design-Verdict` to the verdict file; REQUEST_CHANGES loops back to phase 3 | `.claude/skills/reviewing-figma-designs/SKILL.md`; `.claude/agents/{{REVIEW_BOT}}.md`; model: opus | `Figma-Design-Verdict: APPROVE` in the verdict file (skipped/auto-satisfied if `uiFeature == no`) |
...
| 6 | Plan-review gate | `@{{REVIEW_BOT}}` reviews each child issue, then epic LAST; loops REQUEST_CHANGES→fix→re-dispatch until APPROVED vs comment thread | `.claude/skills/issue-plan-review/SKILL.md`; `.claude/agents/{{REVIEW_BOT}}.md`; model: opus | All issues APPROVED (epic last) |
...
2. **Epic plan review runs LAST** — after all children are APPROVED by `@{{REVIEW_BOT}}` (phase 6).
...
- **Review rubric:** `.claude/skills/reviewing/SKILL.md` — bot-agnostic anti-slop rubric used by all `@{{REVIEW_BOT}}` review gates.
...
This skill is NOT one of the paired no-drift skill copies tracked in `AGENTS.md` § Skill ownership (those are `creating-prs` ↔ user-level `creating-prs` and `reviewing` ↔ user-level `reviewing-as-{{REVIEW_BOT}}`). A user-level `feature-pipeline` namesake does not exist at time of authoring. If one is created later, the no-drift rule applies from that point: changes to either copy must update the other in the same PR, and the PR Summary must say so.
