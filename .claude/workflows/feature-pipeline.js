 ...
 *   4  Figma review      — reviewing-figma-designs + {{REVIEW_BOT}} (UI-only)  [opus]
 *   5  Issue fan-out     — issue-authoring (children first, epic last)        [sonnet]
 *   6  Plan-review loop  — issue-plan-review + {{REVIEW_BOT}} (children first, epic last) [opus]
 ...
 *   9  PR-review loop    — reviewing + {{REVIEW_BOT}} (+ design-reviewer on UI PRs) [opus]
...
export const meta = {
  ...
  description:
    ...
    "pr-workflow, project-bootstrap, {{REVIEW_BOT}}, and design-reviewer as leaf " +
 ...
 * {{REVIEW_BOT}} to review WIP frames. Writes Figma-Design-Verdict to a FILE.
...
async function phase4_figmaReview(args, runState, emit_) {
  ...
  while (attempts < RESOLUTION_MAX_ATTEMPTS) {
    ...
    // Parallel dispatch: reviewing-figma-designs (P9) + {{REVIEW_BOT}} (AC23 analogue for design).
    // Leaf workers — .claude/skills/reviewing-figma-designs/SKILL.md + .claude/agents/{{REVIEW_BOT}}.md (AC21)
    await parallel([
      dispatch({
      ...
      dispatch({
        agent: ".claude/agents/{{REVIEW_BOT}}.md",
...
async function phase6_planReview(args, runState, emit_) {
  ...
  for (const issueNumber of reviewOrder) {
    ...
    while (!approved && attempts < RESOLUTION_MAX_ATTEMPTS) {
      ...
      // Dispatch {{REVIEW_BOT}} loaded with issue-plan-review (AC21).
      await dispatch({
        agent: ".claude/agents/{{REVIEW_BOT}}.md",
 ...
 * parallel() with {{REVIEW_BOT}} on UI PRs only (AC23). Opus.
 *
...
async function phase9_prReview(args, runState, emit_) {
  ...
  for (const [issueNumber, prNumber] of Object.entries(prByIssue)) {
    ...
    while (!converged && attempts < RESOLUTION_MAX_ATTEMPTS) {
      ...
      // On UI PRs: design-reviewer runs in parallel() with {{REVIEW_BOT}} (AC23).
...
async function resolveBlocker(args, runState, emit_, phase, description) {
  ...
  if (attempt > RESOLUTION_MAX_ATTEMPTS) {
    ...
    if (runState.epicIssueNumber) {
      ...
      console.error(
        `[feature-pipeline] HIL ESCALATION (no epic issue yet — phase ${phase}): ${escalationBody}`
      );
    }
